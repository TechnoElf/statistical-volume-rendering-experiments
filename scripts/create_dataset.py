...
def init_grid_with_cloud():
    grid, metadata = vdb.readAll("../run/assets/wdas_cloud_sixteenth.vdb")
    grid = grid[0]
    print(grid.metadata)
...
def init_grid_with_head():
    grid, metadata = vdb.readAll("../run/assets/MRI-Head.vdb")
    grid = grid[0]
    print(grid.metadata)
    grid_acc = grid.getConstAccessor()
    grid_start = grid.metadata["file_bbox_min"]
...
pt_kernel = device.create_compute_kernel(pt_program)

rc_program = device.load_program("raycaster.slang", ["main"])
rc_kernel = device.create_compute_kernel(rc_program)
